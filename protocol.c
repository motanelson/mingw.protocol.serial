...
void ExecuteCommand(char* cmd) {
    ...
    if (textLen > MAX_TEXT+strlen(cmd) +4096) {
            MAX_TEXT=MAX_TEXT+4096+strlen(cmd);
    ...
    strcat(textBuffer,bbuffer);
    f2=fopen(ttys, "w");
    fputs(bbuffer,f2);    
    fputs("\r\n",f2);
    fclose (f2);
    if ( (pipes =fopen(ttys, "r"))==NULL) {
            textLen=strlen(textBuffer);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CHAR: {
        ...
        if (ccc==27){
            bbuffer[0]=0;
            f1=fopen(protocols,"r");
            while(1){
               fgets(bbuffer,4001,f1);
               strcat(bbuffer,"\r\n");
